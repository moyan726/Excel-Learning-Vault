...
**底部明细区（产品成交情况）：** 按产品期数（1期、3期、6期、9期、12期、18期）展开，同样展示成交金额、占比、月环比、成交客户数及客单价。
...
**MODE** 返回众数（出现频率最高的值），**STDEV / STDEVP** 计算标准差（衡量数据离散程度），**PERCENTILE** 返回指定百分位的值（如 `=PERCENTILE(E2:E100, 0.9)` 返回第 90 百分位值），这些在需要做更深入统计分析时会用到。
...
若 H4 为 5，当前年份为 2026，结果为：**"第5月销售数据汇总（2026年）"**。
...
色阶适合**结构型指标**（比例、占比），因为占比数据通常落在一个比较集中的区间内（如 5%~30%），数值差异不大。如果用数据条，条长看起来差异可能不够明显；但用色阶的深浅渐变，能更快帮你识别出哪些是"高占比区域"、哪些是"低占比区域"，观察整体分布是否均衡。
...
**第一段 `[蓝色][>=1]0%`：** 当单元格的值 ≥ 1 时，用蓝色字体显示百分比。例如环比公式计算的结果是"本期/上期"，如果本期比上期增长了 32%，公式返回值为 1.32，此处会显示为蓝色的 `132%`。

**第二段 `[红色][<1]-0%`：** 当值 < 1 时，用红色字体显示，并强制在前面加负号。例如值为 0.83（表示本期是上期的 83%，下降了 17%），显示为红色的 `-17%`。注意这里的负号是格式强制加上的显示符号，不是数学意义上的负数。
